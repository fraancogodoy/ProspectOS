...
def atualizar_configuracao():
    ...
    if chave not in db.CHAVES_CONFIG_VALIDAS:
        return jsonify({"erro": f"clave inválida. Usá una de: {', '.join(db.CHAVES_CONFIG_VALIDAS)}"}), 400
    if not valor:
        return jsonify({"erro": "informá un valor para la clave de API"}), 400
...
def salvar_fonte_maps():
    ...
    if fonte not in FONTES_MAPS_VALIDAS:
        return jsonify({"erro": f"fuente inválida. Usá una de: {', '.join(sorted(FONTES_MAPS_VALIDAS))}"}), 400

    if fonte == "places":
        chave = chave_nova or db.obter_config("places")
        if not chave:
            return jsonify({"erro": "informá la clave de la Google Places API para usar esta fuente"}), 400
...
def salvar_perfil_vendedor():
    ...
    for chave, maximo in CAMPOS_PERFIL_VENDEDOR.items():
        if len(valores[chave]) > maximo:
            return jsonify({"erro": f"campo muy largo (máximo {maximo} caracteres)"}), 400
...
def criar_template():
    ...
    if not titulo:
        return jsonify({"erro": "informá un título para la plantilla"}), 400
    if len(titulo) > MAX_CARACTERES_TITULO_TEMPLATE:
        return jsonify({"erro": f"título muy largo (máximo {MAX_CARACTERES_TITULO_TEMPLATE} caracteres)"}), 400
    if not texto:
        return jsonify({"erro": "informá el texto de la plantilla"}), 400
    if len(texto) > MAX_CARACTERES_TEXTO_TEMPLATE:
        return jsonify({"erro": f"texto muy largo (máximo {MAX_CARACTERES_TEXTO_TEMPLATE} caracteres)"}), 400
...
def atualizar_template(template_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            return jsonify({"erro": "plantilla no encontrada"}), 404
    finally:
        conexao.close()
